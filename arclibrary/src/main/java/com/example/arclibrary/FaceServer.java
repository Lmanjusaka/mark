...
import com.example.arclibrary.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean init(Context context){
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_IMAGE, FaceEngine.ASF_OP_0_HIGHER_EXT,
                        16, 1,
    ...
    private void initFaceList(Context context){
        synchronized (this) {
            ...
            File featureDire = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            if (!featureDire.exists() || !featureDire.isDirectory()) {
                return;
            }
            File[] featureFiles = featureDire.listFiles();
    ...
    private static Rect getBestRect(int width, int height, Rect srcRect) {
        ...
        if (rect.right > width) {
            tempOverFlow = -rect.right;
            if (tempOverFlow > maxOverFlow) {
                maxOverFlow = tempOverFlow;
            }
        }
        if (rect.bottom > height) {
            tempOverFlow = -rect.bottom;
            if (tempOverFlow > maxOverFlow) {
                maxOverFlow = tempOverFlow;
            }
        }
